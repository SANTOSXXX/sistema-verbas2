...
st.set_page_config(page_title="Gestão de Verbas (Link Direto)", layout="wide")
...
# --- CONEXÃO DIRETA ---
# Aqui ignoramos os secrets e passamos o link direto na criação da conexão
conn = st.connection("gsheets", type=GSheetsConnection)

def carregar_dados(aba):
    """Lê uma aba específica forçando o link direto"""
    try:
        if "COLE_SEU_LINK" in LINK_DA_PLANILHA:
            st.error("🚨 Você esqueceu de colar o link na linha 22 do código!")
            return pd.DataFrame()
            
        # ttl=0 garante que não pegamos dados velhos do cache
        # O segredo está aqui: passamos o spreadsheet=LINK explicitamente
        df = conn.read(spreadsheet=LINK_DA_PLANILHA, worksheet=aba, ttl=0)
        return df
...
def salvar_registro(aba, novo_dado_dict):
    try:
        ...
        conn.update(spreadsheet=LINK_DA_PLANILHA, worksheet=aba, data=df_atualizado)
...
def login():
    st.markdown("<h1 style='text-align: center;'>☁️ Gestão Web (Direta)</h1>", unsafe_allow_html=True)
...
if not st.session_state.logged_in:
    ...
    with tab1:
        if not df_lanc.empty:
            if 'data' in df_lanc.columns: df_lanc['data'] = pd.to_datetime(df_lanc['data'], errors='coerce').dt.date
            if 'valor' in df_lanc.columns: df_lanc['valor'] = pd.to_numeric(df_lanc['valor'], errors='coerce').fillna(0)
            
            c1, c2 = st.columns([1, 1])
            anos = sorted(df_lanc["ano"].unique()) if "ano" in df_lanc.columns else [datetime.now().year]
            meses = sorted(df_lanc["mes"].unique()) if "mes" in df_lanc.columns else [datetime.now().month]
        ...
        else: st.info("Planilha 'lancamentos' vazia ou não lida.")
    ...
    with tab2:
        ...
        if cod_input and not df_clientes.empty and "Codigo" in df_clientes.columns:
            ...
            cli = df_clientes[df_clientes['Codigo'] == str(cod_input).strip()]
            if not cli.empty:
                nome_auto, setor_auto = cli.iloc[0]['Nome_Fantasia'], cli.iloc[0]['Setor']
                st.success(f"Cliente: {nome_auto}")
            else: st.warning("Cliente não encontrado.")

        with st.form("form_lcto", clear_on_submit=True):
            c1, c2, c3 = st.columns(3)
            dt = c1.date_input("Data", datetime.now())
            idx_gv, dis_gv = (0, True) if user_perm == "GV 1" else (1, True) if user_perm == "GV 2" else (0, False)
            ...
            if st.form_submit_button("Salvar na Nuvem"):
                if val > 0:
                    novo_reg = {
                        "data": str(dt), "mes": dt.month, "ano": dt.year, "gv": gv, "categoria": cat, 
                        "valor": val, "negociacao": obs, "codigo_cliente": cod_input, "cliente": cli_nome, "setor": setor
                    }
