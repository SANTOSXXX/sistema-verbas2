...
def carregar_dados(aba):
    ...
    try:
        # ttl=0 garante que não pegamos dados velhos do cache ao atualizar
        df = conn.read(worksheet=aba, ttl=0)
        return df
    except Exception as e:
        st.error(f"Erro ao conectar com Google Sheets ({aba}): {e}")
        return pd.DataFrame()
...
if not st.session_state.logged_in:
    ...
    with tab1:
        if not df_lanc.empty:
            ...
            # Totais
            tot_c = dff[dff["categoria"]=="Cerveja"]["valor"].sum()
            tot_n = dff[dff["categoria"]=="NAB (Refrigerantes)"]["valor"].sum()
            ...
            st.info("A planilha de lançamentos parece vazia ou não conectou.")

    # --- TAB 2: LANÇAMENTO ---
    with tab2:
        st.header("Novo Lançamento")
        
        # Busca Cliente (usando Pandas agora)
        col_b, _ = st.columns([1, 2])
        cod_input = col_b.text_input("Cód. Cliente")
        nome_auto, setor_auto = "", ""
        
        if cod_input and not df_clientes.empty:
    ...
    # --- TAB 3: GERENCIAR (Com exclusão na Planilha) ---
    with tab3:
        st.header("Gerenciar Lançamentos")
        if not df_lanc.empty:
            st.dataframe(df_lanc.tail(20), use_container_width=True) # Mostra ultimos 20
            
            idx_del = st.number_input("Digite o número do INDEX (primeira coluna) para excluir:", min_value=0, step=1)
            if st.button("🗑️ Excluir Registro"):
                if idx_del in df_lanc.index:
                    excluir_registro("lancamentos", idx_del)
                    st.success("Registro excluído da planilha!")
                    time.sleep(1)
                    st.rerun()
                else:
