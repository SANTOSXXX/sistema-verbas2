...
def carregar_dados(aba):
    ...
    try:
        ...
        # Mensagem amigável se a aba não for encontrada
        st.error(f"⚠️ Erro ao ler a aba '{aba}'. Verifique se ela existe na planilha com este nome exato.")
        return pd.DataFrame()
...
def salvar_registro(aba, novo_dado_dict):
    ...
    try:
        ...
        # Garante que colunas vazias não quebrem a concatenação
...
if not st.session_state.logged_in:
    ...
    # Carrega dados iniciais das 3 abas
    ...
    with tab1:
        if not df_lanc.empty:
            # Tratamento de dados para evitar erros de tipos
            ...
            # Se a planilha for nova e não tiver coluna 'ano', usa o ano atual
            ...
            # Totais
            tot_c = dff[dff["categoria"]=="Cerveja"]["valor"].sum() if not dff.empty else 0.0
            tot_n = dff[dff["categoria"]=="NAB (Refrigerantes)"]["valor"].sum() if not dff.empty else 0.0
            ...
            st.info("A planilha 'lancamentos' está vazia ou não foi encontrada.")
    ...
    with tab2:
        ...
        if cod_input and not df_clientes.empty and "Codigo" in df_clientes.columns:
            ...
            cli = df_clientes[df_clientes['Codigo'] == str(cod_input)]
            if not cli.empty:
    ...
    with tab3:
        ...
        if not df_lanc.empty:
            st.dataframe(df_lanc.tail(20), use_container_width=True)
            
            idx_del = st.number_input("Digite o número do ÍNDICE (primeira coluna) para excluir:", min_value=0, step=1)
